...
public class QuickSelect<T extends Comparable<T>> {
	...
	private T sort(T[] array, int leftIndex, int rightIndex, int k) {
		...
		if (leftIndex < rightIndex) {
			...
			if (pivot > k - 1)
				return sort(array, leftIndex, pivot - 1, k);
			else if (pivot < k - 1)
				return sort(array, pivot + 1, rightIndex, k);
		}
		return array[k - 1];
	}
	...
	private void medianaPivot(T[] array, int leftIndex, int rightIndex) {
